-- Agendamento por produto e validação multitenant no servidor.
alter table public.produtos
  add column if not exists antecedencia_minutos integer not null default 0
  check (antecedencia_minutos between 0 and 43200);

alter table public.pedidos
  add column if not exists agendado_para timestamptz;

drop function if exists public.criar_pedido_seguro(
  uuid, text, text, text, text, text, uuid, jsonb, text
);

create function public.criar_pedido_seguro(
  p_empresa_id uuid,
  p_modalidade text,
...
declare
  ...
  v_min_antecedencia integer := 0;
  v_max_antecedencia integer := 0;
begin
  ...
  if auth.uid() is null or p_cliente_id is null then raise exception 'Cliente autenticado obrigatório'; end if;
  ...
  with requested as (
    ...
    select p.id, p.nome, n.quantidade, p.preco::numeric(10,2) as preco,
           coalesce(p.antecedencia_minutos, 0) as antecedencia_minutos
  ...
  select jsonb_agg(jsonb_build_object(
         ...
         min(antecedencia_minutos), max(antecedencia_minutos)
    into v_itens, v_subtotal, v_min_antecedencia, v_max_antecedencia from priced;
  ...
  if v_min_antecedencia = 0 and v_max_antecedencia > 0 then
    raise exception 'Produtos de entrega imediata e produtos agendados devem ser feitos em pedidos separados';
  end if;
  if v_max_antecedencia > 0 then
    if p_agendado_para is null then raise exception 'Escolha a data e o horário do pedido agendado'; end if;
    if p_agendado_para < now() + make_interval(mins => v_max_antecedencia) then
      raise exception 'O horário escolhido não respeita a antecedência mínima';
    end if;
    if p_agendado_para > now() + interval '365 days' then raise exception 'Agendamento muito distante'; end if;
  elsif p_agendado_para is not null then
    raise exception 'Produtos comuns não precisam de agendamento';
  end if;

  if p_modalidade = 'entrega' then
    if coalesce((v_empresa.entrega->>'entregaAtiva')::boolean, false) is false then raise exception 'Entrega indisponível'; end if;
  ...
  insert into public.pedidos (
    ...
    subtotal, taxa_entrega, total, agendado_para
  ) values (
    ...
    case when v_max_antecedencia > 0 then p_agendado_para else null end
...
revoke all on function public.criar_pedido_seguro(
  uuid, text, text, text, text, text, uuid, jsonb, text, timestamptz
) from public, anon, authenticated;
grant execute on function public.criar_pedido_seguro(
  uuid, text, text, text, text, text, uuid, jsonb, text, timestamptz
) to authenticated;

grant update (status, estimativa_minutos, previsao_em, agendado_para) on public.pedidos to authenticated;
