  ...
  return (
    <section>
      <AdminSectionHeader eyebrow="Preferências" title="Configurações"
        description="Configure a identidade visual, dados operacionais e horário de funcionamento."
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <p className="mb-5 text-sm text-slate-500">Sua cor e logo aparecerão em destaque no cardápio. O estilo define o clima visual sem mudar as suas cores.</p>
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <label className="block text-sm font-bold text-slate-700">
              ...
              <div className="mt-2 flex items-center gap-3">
                <input type="color" value={form.corPrincipal} onChange={(e) => setForm({ ...form, corPrincipal: e.target.value })}
                  className="h-12 w-16 cursor-pointer rounded-2xl border border-slate-200 p-1 outline-none"
                />
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 p-4">
              ...
              <div className="overflow-hidden rounded-2xl shadow-md" style={{ background: brandPreview.heroGradient }}>
                <div className="flex items-center gap-3 p-4">
                  {form.logoUrl ? (
                    <img src={form.logoUrl} alt="logo" className="h-10 w-10 rounded-xl bg-white object-cover p-0.5" />
                  ) : (
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/20 text-xs font-black text-white">Logo</div>
                  )}
                  <div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <label className="block text-sm font-bold text-slate-700">
              Nome da empresa
              <input value={form.nomeEmpresa} onChange={(e) => setForm({ ...form, nomeEmpresa: e.target.value })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Cidade/UF
              <input value={form.cidadeUf} onChange={(e) => setForm({ ...form, cidadeUf: e.target.value })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
            <label className="block text-sm font-bold text-slate-700 lg:col-span-2">
              Descrição
              <textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} rows={3} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Telefone WhatsApp
              <input value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Taxa de entrega
              <input type="number" min="0" step="0.01" value={form.taxaEntrega} onChange={(e) => setForm({ ...form, taxaEntrega: e.target.value })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Pedido mínimo
              <input type="number" min="0" step="0.01" value={form.pedidoMinimo} onChange={(e) => setForm({ ...form, pedidoMinimo: e.target.value })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <label className="block text-sm font-bold text-slate-700">
              Status da loja
              <select value={form.statusLoja} onChange={(e) => setForm({ ...form, statusLoja: e.target.value as StatusLoja })} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400">
                {(Object.entries(statusLojaLabels) as [StatusLoja, string][]).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Mensagem ao cliente
              <input value={form.mensagemCliente} onChange={(e) => setForm({ ...form, mensagemCliente: e.target.value })} placeholder="Ex: Hoje com atraso de 20 minutos." className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-400" />
            </label>
          </div>
          <div className={`mt-5 rounded-2xl px-4 py-3 text-sm font-bold ${
            form.statusLoja === 'forcar_aberto' ? 'bg-emerald-50 text-emerald-700' :
            form.statusLoja === 'forcar_fechado' ? 'bg-red-50 text-red-700' : 'bg-slate-50 text-slate-600'
          }`}>
            ...
            {diasSemana.map((dia) => (
              <div key={dia.key} className={`grid gap-3 rounded-2xl border p-4 ${ form.dias[dia.key].ativo ? 'border-slate-200 bg-white' : 'border-slate-100 bg-slate-50' } md:grid-cols-[140px_100px_1fr_1fr] md:items-center`}>
                ...
                <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
                  <input type="checkbox" checked={form.dias[dia.key].ativo} onChange={(e) => updateDay(dia.key, 'ativo', e.target.checked)} className="h-4 w-4" />
                  {form.dias[dia.key].ativo ? 'Ativo' : 'Fechado'}
                </label>
                <label className="block text-sm font-bold text-slate-700">Abertura
                  <input type="time" value={form.dias[dia.key].abertura} onChange={(e) => updateDay(dia.key, 'abertura', e.target.value)} disabled={!form.dias[dia.key].ativo} className="mt-1 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none disabled:cursor-not-allowed disabled:bg-slate-100" />
                </label>
                <label className="block text-sm font-bold text-slate-700">Fechamento
                  <input type="time" value={form.dias[dia.key].fechamento} onChange={(e) => updateDay(dia.key, 'fechamento', e.target.value)} disabled={!form.dias[dia.key].ativo} className="mt-1 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none disabled:cursor-not-allowed disabled:bg-slate-100" />
                </label>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button type="submit" className="rounded-full bg-slate-950 px-6 py-3 font-black text-white hover:bg-slate-800">
