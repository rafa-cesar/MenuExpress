      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
            ...
            {categorias.map((category) => (
              <article key={category.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => removeCategory(category.id)} className="rounded-full bg-red-50 px-4 py-2 text-sm font-black text-red-600 hover:bg-red-100">
                    Remover
                  </button>
                </div>
              </article>
        ...
        <form onSubmit={handleSubmit} className="h-fit rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-4">
            ...
            <button type="submit" className="w-full rounded-full bg-brand-600 px-5 py-3 font-black text-white hover:bg-brand-700">Cadastrar categoria</button>
          </div>
        </form>
      </div>
    </section>
