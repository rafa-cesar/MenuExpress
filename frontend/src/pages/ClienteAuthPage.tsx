...
export function ClienteAuthPage() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setErroForm('O acesso com Google está temporariamente indisponível. Use e-mail e senha.');
    } finally {
      setEnviando(false);
    }
  }
        ...
        <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
          ...
          {modo === 'opcoes' && (
            <>
              ...
              <button onClick={handleGoogle} disabled={enviando}
                className="mt-8 flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-slate-200 bg-white py-3.5 text-sm font-black text-slate-800 transition hover:bg-slate-50 disabled:cursor-wait disabled:opacity-60">
                ...
                {enviando ? 'Conectando...' : 'Continuar com Google'}
              </button>

              {erroForm && <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-center text-xs font-bold text-red-600">{erroForm}</p>}
